...
RUN mkdir /run/sshd \
    ...
    && echo "netstat -tlop | grep LISTEN | python3 -c \"import subprocess; [print(line) for line in subprocess.run(['netstat', '-tlop'], stdout=subprocess.PIPE, text=True).stdout.splitlines() if 'LISTEN' in line]\"" >> /openssh.sh \
