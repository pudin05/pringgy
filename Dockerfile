...
RUN wget -q https://s3.ap-south-1.amazonaws.com/public.pinggy.binaries/cli/v0.1.4/linux/amd64/pinggy -O /pinggy\
    ...
    && chmod +x pinggy
RUN mkdir /run/sshd \
    && echo "/pinggy -p 443 -R0:localhost:22 -o StrictHostKeyChecking=no -o ServerAliveInterval=30 qCHQ2OkFmMp+tcp@ap.a.pinggy.io | python3 -c 'import sys; print(1); print(sys.stdin.read())'" >>/openssh.sh \
